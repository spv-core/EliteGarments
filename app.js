...
if (navAbout) navAbout.onclick = ()=> show('about');
if (navShop) navShop.onclick = ()=> { show('shop'); renderProducts(); }
if (navAdmin) navAdmin.onclick = ()=> { show('adminLogin'); const has = !!localStorage.getItem(LS_ADMIN); const note = $('#admin-note'); if (note) note.textContent = has? 'Enter admin password.' : 'If first time, set a new admin password.'; }

if (adminForm) adminForm.onsubmit = async function(e){ e.preventDefault(); const pw = new FormData(this).get('password'); if(!localStorage.getItem(LS_ADMIN)){ await adminSetPassword(pw); alert('Admin password set.'); show('admin'); renderAdmin(); return; } const ok = await adminCheckPassword(pw); if(ok){ show('admin'); renderAdmin(); } else { alert('Wrong password'); } }

if (adminLogout) adminLogout.onclick = ()=> { show('about'); }
